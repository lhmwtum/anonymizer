...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "Path to a folder that contains the images that should be"
            " anonymized. Images can be arbitrarily nested in subfolders and"
            " will still be found."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Path to the folder the anonymized images should be written to. "
    ...
    parser.add_argument(
        ...
        help=(
            "Path to the folder where the weights are stored. If no weights"
            " with the appropriate names are found they will be downloaded"
            " automatically."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="0.3",
        help=(
            "Detection confidence needed to anonymize a detected face. "
    ...
    parser.add_argument(
        ...
        metavar="0.3",
        help=(
            "Detection confidence needed to anonymize a detected license plate."
            " Must be in [0.001, 1.0]"
        ),
    )
    parser.add_argument(
        "--write-detections", dest="write_detections", action="store_true"
    )
    parser.add_argument(
        "--no-write-detections", dest="write_detections", action="store_false"
    )
    ...
    parser.add_argument(
        ...
        help=(
            "This parameter is used to change the way the blurring is done. For"
            " blurring a gaussian kernel is used. The default size of the"
            " kernel is 21 pixels and the default value for the standard"
            " deviation of the distribution is 2. Higher values of the first"
            " parameter lead to slower transitions while blurring and larger"
            " values of the second parameter lead to sharper edges and less"
            " blurring. To make the transition from blurred areas to the"
            " non-blurred image smoother another kernel is used which has a"
            " default size of 9. Larger values lead to a smoother transition."
            " Both kernel sizes must be odd numbers."
        ),
    )
