...
class Detector:
    def __init__(self, kind, weights_path):
        ...
        self.session = tf.compat.v1.Session(
            graph=self.detection_graph, config=conf
        )
    ...
    def _convert_boxes(
        ...
        ), 'Threshold can not be too close to "0".'
    ...
    def detect(self, image, detection_threshold):
        ...
        assert channels == 3, (
            ...
            "Only images with three color channels are supported."
        )
        ...
        converted_boxes = self._convert_boxes(
